            ...
            <tbody className="tbody">
              {ArrayData.map((item, index) => (
                ...
                >
                  ...
                  <th
                    scope="row"
                    className="px-6 py-4 font-medium whitespace-nowrap text-[16px]"
                  >
                    {item.Title}
                  </th>
                  ...
                  <div className="relative p-6 flex-auto">
                    <p className="my-4 text-blueGray-500 flex flex-col text-lg leading-relaxed">
                      <span className="flex justify-between m-auto flex-row">
                        ...
                        />
                        <img src={ShowImage} className="w-20 h-32" alt="img not found" />
                      </span>
                      <input
                        type="text"
                        className="border-2 border-black p-2 m-6 rounded"
...
const ProductCard = ({ item, convertToBase64 }) => {
    ...
    return (
        <div>
            <h2>{item.name}</h2>
            {imageSrc ? <img src={imageSrc} className="w-32 h-32" alt={item.name} /> : <p>Loading image...</p>}
        </div>
    );
};
